...
Status: design record, pre-alpha, kept in step with the code. The first version was written before any code; the second recorded the run seam after review round one; this one records the boundary and the control-first order after review round two.
...
**The scope** says which fixture scopes are rebuilt under the change: function (the default), class, module or session. It is the teardown boundary for every run of the check, so a verdict never depends on what pytest happens to run next, in serial or under xdist. A check that requests a fixture wider than its declaration's scope is never accused of being false when the negative run passes: the change may never have reached that fixture, and the verdict is unproven with the fixture and the scope named.
...
| passes | passes | passes, and the check uses a fixture wider than the scope | **unproven** | out-of-scope |
...
| any | | | **unproven** | misconfigured (a `no_proof` marker without a reason, or beside a declaration) |
