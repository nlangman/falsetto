...
The plugin is inert unless enabled with `--falsetto`, `--falsetto-strict`, or the `falsetto` and `falsetto_strict` ini keys. When enabled it takes over the run protocol for function-based items, including `unittest.TestCase` methods. Every run of a declared check is a whole unlogged protocol (setup, call, teardown) torn down to the declaration's boundary; after the runs, the fixture stack is torn down to what the real next item needs, and a failure there becomes a teardown report. Each run is observed through its reports, which is what makes `unittest` items grade correctly. The exception behind a failed call is captured from the report hook for the expectation check. The positive run's reports are logged once, with the verdict attached as one JSON record on the call and teardown reports' `user_properties`, appended last, so it survives serialization under xdist and appears in JUnit output as a parseable property. No record under one of Falsetto's own names reaches a reader from anywhere else. As each report is made, the report hook strips every `falsetto.verdict` and `falsetto.excluded` entry from it, and Falsetto appends its own afterwards and only for an item it graded. The stripping runs where the report is made, which is the worker under xdist, so the only such record that crosses the process boundary is Falsetto's, and no secret has to cross it for the controller to trust one. What survives is still validated before it is counted, the verdict and the reason must be members of the enums, and the sentence and the hint are rebuilt from the reason, and it is still read last-wins, so neither guard stands alone and a malformed record is ignored rather than counted or crashed on. This closes what was an open limit: an item Falsetto did not grade, one excluded by `no_proof` or skipped, xfail, or run by another plugin, has no record of Falsetto's to win over a forged one, and now has no forged one either. A record a test wrote under those two names is dropped rather than renamed, so it no longer appears in JUnit output either; the names are Falsetto's, and a copy kept under one of them reads as a verdict to everything downstream. A check that wants a property of its own should use a name outside the `falsetto.` prefix, which is left alone. The one path this does not cover is a plugin that builds reports itself, without pytest's report hook, and copies the item's properties into them.

The control and negative runs execute with coverage measurement paused, so a hand-written stub cannot inflate a coverage gate, and with the `--pdb` and `--trace` plugins unregistered, so a deliberate failure never opens a debugger; other consumers of `pytest_exception_interact` still see those failures. A timeout plugin that arms one budget per protocol spends it across all runs. `--durations` reports the positive run only. After the graded runs the fixture stack is torn down to what the real next item needs, or entirely when the session is about to stop, by the same call pytest's teardown hook makes; that final teardown does not re-enter per-phase plugin hooks, and a failure in it is merged into the check's teardown report, so it is one error on one JUnit testcase and is counted in the verdict line. Captured output of the control and negative runs is not logged; the failure text of a run that decided a verdict travels as evidence in the verdict record and is printed in the summary. Doctests and custom item types are left to pytest and counted as not gradable. `@pytest.mark.no_proof(reason)` excludes a check; the reason is required, the check is listed by name, and it is counted as excluded, never as a pass. `--falsetto-json PATH` writes every verdict, with reason codes, messages, hints, details, evidence and whether that verdict fails the build, plus the totals (verdict counts and status counts kept apart), the not-graded items, whether the session ran to completion or stopped early and why, and the run's context: a schema number, the root directory the relative paths hang off, the invocation arguments, strict mode, the control count, the pytest and Python versions, and the start and finish times. The directory is created if needed, and a report that cannot be written is an internal-error exit, never a green build with a missing channel. A prototype on 2026-09-17 (three fresh agents, each given only a report and the directory it described) found that the reason codes, the out-of-scope detail and the control run's evidence carried the diagnosis on their own; per-check source locations are deferred until a suite large enough to need them can measure them.
