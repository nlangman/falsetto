...
criterion's stated assumption, and runs the criterion's test against the mutant. A test
that still passes is marked `VACUOUS`; a criterion that cannot be validated
deterministically is marked `UNVALIDATABLE` and needs human sign-off.
...
- Falsetto reports four states as native runner outcomes, caches proofs, and extends
  the same rule to evals.

### pytest-mutagen (2020)

[pytest-mutagen](https://pypi.org/project/pytest-mutagen/) (version 1.3, released
2020-07-24; Timothee Paquatte and Harrison Goldstein, described in
...
- Mutants belong to files or functions and are applied to the whole suite. Falsetto's
...
Niedermayr and others introduced extreme mutation: delete a covered method's whole
body, or replace it with a trivial return, and see whether any test fails. A method the
suite executes but whose removal changes no test outcome is *pseudo-tested*.
...
[PseudoSweep](https://philmcminn.com/publications/maton2024b.pdf) (Maton and McMinn,
2024) extends it to statements. Studies report pseudo-tested methods are common: on the
order of a tenth of methods under unit tests and a third under system tests.
...
- **Mutation feedback for test generation.** MuTAP, MutGen and GEM feed surviving
  mutants back into a language model so it generates stronger tests. They improve
  generation; they do not attach a declared change to each test.
...
- **Evidence that the problem is real.** Recent studies of language-model-generated
  tests distinguish *fault triggering* (the test executes the faulty line) from *fault
  detection* (the test fails because of it), and report detection rates far below
  triggering rates. A test that triggers a fault and passes is exactly a **false**
  check.
...
2. **A per-check verdict with a third and fourth state**, reported as native runner
   outcomes, so "this specific check is wrong" is a build result, not a metric.
...
- spec-verify and pytest-mutagen: the linked pages were read directly on 2026-09-14.
- Descartes, PseudoSweep and the extreme-mutation studies: abstracts and published
  summaries read; the papers' figures quoted above are as those summaries report them.
- The wider-field entries: characterizations from documentation and search summaries;
  to be re-read before the first public release, as `docs/plan.md` requires.
